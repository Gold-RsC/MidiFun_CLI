...
int main(int argc, char** argv) {
    ...
    app.add_subcommand("version", "Get version")->callback([&] { std::cout << "Version: " VERSION << std::endl; });
        ...
        void setup(CLI::App& app) {
            ...
            subcommand->callback([&] {
                ...
                for (size_t i = 0; i < tracks_microsecond.size(); ++i) {
                    ...
                    track_time_min_max[i] = {std::numeric_limits<MidiTime>::max(),
                ...
                tracks_microsecond.for_event([&track_time_min_max, &note_time_min_max](const MidiEvent& event) {
                    ...
                    track_time_min_max[event.track].second =
                        std::max(track_time_min_max[event.track].second, event.time);
                    if (event.type() == MidiEventType::note_on || event.type() == MidiEventType::note_off) {
                        note_time_min_max[event.track].first =
                            std::min(note_time_min_max[event.track].first, event.time);
                        note_time_min_max[event.track].second =
                            std::max(note_time_min_max[event.track].second, event.time);
                    }
                });
                ...
                if (print_time) {
                    std::cout << "    Song Time:                   "
                              << progress_bar(song_time_min_max, song_time_min_max) << std::endl;
                    ...
                    for (MidiTrackList::const_iterator track_it = tracks_microsecond.cbegin();
                        ...
                        if (print_bpm && trackIdx < bpmMap.size() && !bpmMap[trackIdx].empty()) {
                            if (print_time) {
                                ...
                                for (auto it = bpmMap[trackIdx].begin() + 1; it != bpmMap[trackIdx].end(); ++it) {
                                    std::cout << "                                " << std::fixed
                                              << std::setprecision(2) << it->bpm << str_time(it->time, time_mode)
                                              << std::endl;
                                }
                            }
                        }
                            ...
                            for (const Text& text : parser.textMap[trackIdx]) {
                                if (text_type_set.empty() || text_type_set.find(text.type) != text_type_set.end()) {
                                    std::cout << "        " << std::setw(25) << std::setfill(' ') << std::left
                                              << ("Text (" + Text::get_typeName(text.type) + "):")
                                              << safe_print(text.text) << std::endl;
                                }
                            }
                        }
                    }
    ...
    info_subcommand_t info_subcommand;
    info_subcommand.setup(app);

    struct get_note_subcommand_t {
        std::string filepath;
        MidiTimeMode time_mode;
    ...
    get_note_subcommand_t get_note_subcommand;
    get_note_subcommand.setup(app);
    struct get_notepair_subcommand_t {
        std::string filepath;
        MidiTimeMode time_mode;
        bool print_label = false;
    ...
    get_notepair_subcommand_t get_notepair_subcommand;
    get_notepair_subcommand.setup(app);
    struct play_subcommand_t {
        void setup(CLI::App& app) {
